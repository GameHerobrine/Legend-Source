...
//            MovementInput, GameSettings, KeyBinding, AliasGlobal, 
//            EntityPlayer
...
public class MovementInputFromOptions extends MovementInput
{
    ...
    public void checkKeyForMovementInput(int i, boolean flag)
    {
        ...
        LegendGlobal.checkKeys(i, flag);
        if(i == gameSettings.keyBindSneak.keyCode && (LegendGlobal.Fly || LegendGlobal.freeCam))
        {
            flag = false;
        }
        if(byte0 >= 0)
    ...
    public void updatePlayerMoveState(EntityPlayer entityplayer)
    {
        moveStrafe = 0.0F;
        moveForward = 0.0F;
        if(movementKeyStates[0] || LegendGlobal.autowalk)
        {
            moveForward++;
        }
