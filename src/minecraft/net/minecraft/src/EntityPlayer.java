...
//            GuiIngame, MathHelper, AxisAlignedBB, Entity, 
//            ItemStack, Item, EntityItem, Material, 
//            NBTTagCompound, NBTTagList, EntityMob, EntityArrow, 
//            EntityCreeper, EntityGhast, EntityWolf, AliasGlobal, 
//            EnumStatus, WorldProvider, BlockBed, Block, 
//            IChunkProvider, EntityMinecart, AchievementList, EntityBoat, 
//            EntityPig, EntityFish, IInventory, TileEntityFurnace, 
//            TileEntityDispenser, TileEntitySign, StatBase
...
public abstract class EntityPlayer extends EntityLiving
{
    ...
    {
        if(GuiIngame.NameList.contains(username))
        {
            playerCloakUrl = (new StringBuilder()).append("https://hosting.photobucket.com/images/i/ShaeIsntQt/Stigma2011_0.3.png").toString();
    ...
    public void dropPlayerItemWithRandomChoice(ItemStack itemstack, boolean flag)
    {
        ...
        if(flag)
        {
            ...
            entityitem.motionX = -MathHelper.sin(f3) * f1;
            entityitem.motionZ = MathHelper.cos(f3) * f1;
            entityitem.motionY = 0.20000000298023224D;
        } else
        {
            float f2 = 0.3F;
            entityitem.motionX = -MathHelper.sin((rotationYaw / 180F) * 3.141593F) * MathHelper.cos((rotationPitch / 180F) * 3.141593F) * f2;
            entityitem.motionZ = MathHelper.cos((rotationYaw / 180F) * 3.141593F) * MathHelper.cos((rotationPitch / 180F) * 3.141593F) * f2;
            entityitem.motionY = -MathHelper.sin((rotationPitch / 180F) * 3.141593F) * f2 + 0.1F;
            f2 = 0.02F;
            float f4 = rand.nextFloat() * 3.141593F * 2.0F;
            f2 *= rand.nextFloat();
            entityitem.motionX += Math.cos(f4) * (double)f2;
            entityitem.motionY += (rand.nextFloat() - rand.nextFloat()) * 0.1F;
            entityitem.motionZ += Math.sin(f4) * (double)f2;
        }
