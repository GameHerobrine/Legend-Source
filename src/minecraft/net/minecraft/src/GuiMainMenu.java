...
//            GuiScreen, AliasGlobal, LogoEffectRandomizer, StringTranslate, 
//            GuiButton, GuiOptions, GuiSelectWorld, GuiMultiplayer, 
//            GuiTexturePacks, Tessellator, RenderEngine, MathHelper, 
//            fontRenderer, ScaledResolution, RenderBlocks, Block
...
public class GuiMainMenu extends GuiScreen
{
    ...
    public GuiMainMenu()
    {
        ...
        if(!load)
        {
            LegendGlobal.loadNameList();
            LegendGlobal.getRealUsername();
            load = true;
        }
        try
        {
    ...
    public void drawScreen(int i, int j, float f)
    {
        ...
        int k = width / 2 - c / 2;
        byte byte0 = 30;
        GL11.glBindTexture(3553 /*GL_TEXTURE_2D*/, mc.renderEngine.getTexture("/title/black1.png"));
        ...
        drawCenteredString(fontRenderer, splashText, 0, -8, 0xCECECE);
        GL11.glPopMatrix();
        drawString(fontRenderer, net.minecraft.src.me.Shxe.Main.NAME + " - " + net.minecraft.src.me.Shxe.Main.VERSION, 2, 2, 0x00ff00);
        String s = "By Shae";
        drawString(fontRenderer, s, width - fontRenderer.getStringWidth(s) - 2, height - 10, 0x686868);
        super.drawScreen(i, j, f);
    }

    private void drawLogo1(float f)
    {
        if(logoEffects == null)
        {
        ...
        for(int l = 0; l < 3; l++)
        {
            ...
            for(int i1 = 0; i1 < minecraftLogo.length; i1++)
            {
                for(int j1 = 0; j1 < minecraftLogo[i1].length(); j1++)
                {
                    ...
                    renderblocks.func_1238_a(Block.blockGold, f3);
                    GL11.glPopMatrix();
                }
    ...
    private void drawLogo(float f)
    {
        if(logoEffects == null)
        {
            logoEffects = new LogoEffectRandomizer[minecraftborder[0].length()][minecraftborder.length];
            for(int i = 0; i < logoEffects.length; i++)
            {
                for(int j = 0; j < logoEffects[i].length; j++)
                {
        ...
        for(int l = 0; l < 3; l++)
        {
            ...
            GL11.glRotatef(15F, 1.0F, 0.0F, 0.0F);
            GL11.glScalef(0.89F, 1.0F, 0.4F);
            GL11.glTranslatef((float)(-minecraftborder[0].length()) * 0.5F, (float)(-minecraftborder.length) * 0.5F, 0.0F);
            GL11.glBindTexture(3553 /*GL_TEXTURE_2D*/, mc.renderEngine.getTexture("/terrain.png"));
            if(l == 0)
            {
                GL11.glBindTexture(3553 /*GL_TEXTURE_2D*/, mc.renderEngine.getTexture("/title/black.png"));
            }
            for(int i1 = 0; i1 < minecraftborder.length; i1++)
            {
                for(int j1 = 0; j1 < minecraftborder[i1].length(); j1++)
                {
                    char c = minecraftborder[i1].charAt(j1);
                    if(c == ' ')
                    {
                        continue;
                    }
                    ...
                    GL11.glScalef(f2, f2, f2);
                    GL11.glRotatef(f4, 0.0F, 1.0F, 0.0F);
                    renderblocks.func_1238_a(Block.blockDiamond, f3);
                    GL11.glPopMatrix();
                }
