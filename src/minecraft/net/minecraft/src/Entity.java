...
//            AliasGlobal, Block, StepSound, Material, 
...
public abstract class Entity
{
    ...
    public void moveEntity(double d, double d1, double d2)
    {
        ...
        if(stepHeight > 0.0F && flag1 && (flag || ySize < 0.05F) && (d5 != d || d7 != d2))
        {
            ...
            {
                d1 = ((AxisAlignedBB)list1.get(l2)).calculateYOffset(boundingBox, d1);
            }
            ...
            {
                d = ((AxisAlignedBB)list1.get(i3)).calculateXOffset(boundingBox, d);
            }
            ...
            {
                d2 = ((AxisAlignedBB)list1.get(j3)).calculateZOffset(boundingBox, d2);
            }
            ...
            } else
            {
                ...
                {
                    d1 = ((AxisAlignedBB)list1.get(k3)).calculateYOffset(boundingBox, d1);
                }
        ...
        if(canTriggerWalking() && !flag && ridingEntity == null)
        {
            ...
            {
                j2 = worldObj.getBlockId(l, j1 - 1, l1);
            }
            if(distanceWalkedModified > (float)nextStepDistance && j2 > 0)
            {
                nextStepDistance++;
                ...
                if(!Block.blocksList[j2].blockMaterial.getIsLiquid())
                {
                    worldObj.playSoundAtEntity(this, stepsound.func_1145_d(), stepsound.getVolume() * 0.15F, stepsound.getPitch());
                }
                Block.blocksList[j2].onEntityWalking(worldObj, l, j1, l1, this);
            }
        }
        ...
        int k2 = MathHelper.floor_double(boundingBox.maxX - 0.001D);
        int l3 = MathHelper.floor_double(boundingBox.maxY - 0.001D);
        int i4 = MathHelper.floor_double(boundingBox.maxZ - 0.001D);
        if(worldObj.checkChunksExist(i1, k1, i2, k2, l3, i4))
        {
            for(int j4 = i1; j4 <= k2; j4++)
            {
