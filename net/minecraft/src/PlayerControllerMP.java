...
//            ItemStack, AliasGlobal, Packet14BlockDig, NetClientHandler, 
//            Packet130UpdateSign, Block, StepSound, SoundManager, 
//            GuiIngame, RenderGlobal, InventoryPlayer, Packet16BlockItemSwitch, 
//            Packet15Place, EntityClientPlayerMP, Packet7UseEntity, Entity, 
//            Container, Packet102WindowClick
...
public class PlayerControllerMP extends PlayerController
{
    ...
    public void clickBlock(int i, int j, int k, int l)
    {
        ...
        {
            LegendGlobal.autoBreakID = mc.theWorld.getBlockId(i, j, k);
        }
        if(!isHittingBlock || i != currentBlockX || j != currentBlockY || k != currentblockZ)
        {
            ...
            if(LegendGlobal.sign)
            {
                ...
                if(i1 == 68 || i1 == 63)
                {
                    ...
                    as[2] = s2;
                    as[3] = s;
                    mc.getSendQueue().addToSendQueue(new Packet130UpdateSign(i, j, k, as));
                }
            }
            ...
            if(i1 > 0 && Block.blocksList[i1].blockStrength(mc.thePlayer) >= 1.0F || LegendGlobal.instant)
            {
                if(LegendGlobal.instant)
                {
                    for(int j1 = 0; j1 < 1000; j1++)
                    {
                        mc.getSendQueue().addToSendQueue(new Packet14BlockDig(0, i, j, k, 1));
                        mc.getSendQueue().addToSendQueue(new Packet14BlockDig(2, i, j, k, 1));
                    }
    ...
    {
        return LegendGlobal.extendedReach ? 100F : 5F;
    }
